...
class CustomerController extends Controller
{
    ...
    public function customerLogin(Request $request){
        ...
        if(Auth::attempt(['user_email' => $request->user_email, 'password' => $request->user_password])){
            ...
            return redirect('')->with('msgSuccess', 'Đăng nhập thành công');
        }
        else{
    ...
    public function customerRegister(Request $request){
        ...
        try {
            ...
            $verificationUrl = URL::temporarySignedRoute(
                ...
                now()->addMinutes(60), // Thời hạn URL
                ['user_id' => $user->user_id] // Tham số
            );
        
            // Gửi email xác minh
            Mail::to($user->user_email)->send(new VerifyAccount($user, $verificationUrl));
        
            return redirect('customer')->with('msgSuccess', 'Đăng kí thành công. Vui lòng kiểm tra email.');
        } catch (\Throwable $th) {
            Log::error('Error during user registration:', [
                'message' => $th->getMessage(),
                'trace' => $th->getTraceAsString(),
            ]);
            return redirect()->back()->with('msgError', 'Đăng ký không thành công: ' . $th->getMessage());
        }
